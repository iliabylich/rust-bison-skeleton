...
cargo_toml="rust-bison-skeleton/Cargo.toml"
...
function walk() {
	...
	for hash in $(git log --first-parent --format=oneline "$start_hash" -- "$cargo_toml" | awk '{print $1}'); do
		n=$(git show "$hash:$cargo_toml" | grep -E '^name\s*=' | head -n1 | sed -e 's/[^"]*"//' -e 's/".*//')
		v=$(git show "$hash:$cargo_toml" | grep -E '^version\s*=' | head -n1 | sed -e 's/[^"]*"//' -e 's/".*//')
		if [[ -z "$last_name" ]]; then
			last_name=$n
		elif [[ -z "$n" ]]; then
